...
> v0.2 alpha changed the cache location to `%LocalAppData%\winget-frontends\source\winget-pkgs\pkglist\manifests` and the path above will be changed when it's out.
...
- [X] "winget-frontends-pkglist" could be stored in the user's AppData (LocalAppData in v0.2+) folder. Having the location be configurable to be on a USB drive instead for example may make things confusing and result in frontends becoming incompatible or having the incorrect information if they choose to use a different location.

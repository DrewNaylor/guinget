...
echo The purpose of update-manifests is to expediate the downloading and extracting of winget sources from Microsoft's Community Package Repository. For now, this is the only source supported, but the goal is to support more sources in the future, along with winget gaining that feature. UAS will use PowerShell to download a Zip file from github.com, extract it into a temp file in your user profile folder with PowerShell, and build the Visual Basic project file with MSBuild. 

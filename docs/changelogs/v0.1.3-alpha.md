...
- More [experimental options](https://drew-naylor.com/guinget/experimental-options) have been added, and you can use them by clicking in the lower-left corner of the `About` window then opening `Tools>Options...>Experimental`. Please note that they're not fully tested and may cause issues.
...
- Not really a new feature since it wasn't noticeably lacking before, but you can use the mouse scroll wheel to switch tabs in the `Options` window.

...
- Exact matches are now the default when using `Selected packages>Search for package ID`, but you can change it back by unchecking `Use exact match for selected package ID search` in `Tools>Options...>Search`. Since this relies on using double-quotes ("") in the search box, you can surround your search query with double-quotes if you want to do an exact search without using the menu item, even if the option to use exact matches is off.

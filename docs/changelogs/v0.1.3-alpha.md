...
- You can now do things like update winget's sources (if it doesn't do it automatically) and edit winget's settings without typing anything into a command-line.
...
- The `Selected packages` submenu has been moved out of the `Package list` menu to its own menu.
- You can show a package in winget using the `Show in winget` button, available in the package context menu and the `Selected packages` menu.

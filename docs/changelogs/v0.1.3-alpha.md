...
- More [experimental options](https://drew-naylor.com/guinget/experimental-options) have been added, and you can use them by checking `Hide unfinished controls and experimental stuff` on the `Tools>Options...>Experimental` tab, then restarting guinget. Please note that they're not fully tested and may cause issues.

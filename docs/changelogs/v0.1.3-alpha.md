...
Took a half a year, but you can now bulk-install packages! Besides that very important change, you can now do things like update winget's sources (if it doesn't do it automatically) and edit winget's settings without typing anything into a command-line. This version also introduces a checkbox you can use to have winget run elevated when installing packages, something rather important in case winget doesn't automatically elevate the installer like it's supposed to.

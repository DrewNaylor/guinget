...
- An `Options` window has been added. For now, this only has search-related options, but more stuff will be here in a future version. You can get to this window from `Package list>Search options...` or from `Search options...` in the `Search` button dropdown on the toolbar.

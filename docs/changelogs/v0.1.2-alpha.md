...
- Some experimental features have been added, and you can use them by changing the config file
  - `Use7zipForExtraction`: Boolean; defaults to `False`
    - Package manifest ZIP files will be extracted using 7-Zip instead of the built-in .NET extractor. This may be a lot faster, although it's not fully tested nor is it as complete as the code using the .NET extractor.

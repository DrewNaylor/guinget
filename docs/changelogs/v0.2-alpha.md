...
- Specify version number when installing, upgrading, and uninstalling
  ...
  - You can change these from `Tools>Options...>Apply Changes` and the mini-form in the `Apply changes` window accessible from the `Options...` linklabel in the bottom-left corner.

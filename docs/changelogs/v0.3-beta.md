...
- Closing the cache update window now sets the cancel update flag just like if you clicked the "Cancel" button. This has been a problem since v0.1 alpha, but it's now addressed. There's no way to stop it during the moving phase and it'll just continue in the background if the window is closed. ([issue #129](https://github.com/DrewNaylor/guinget/issues/129))
